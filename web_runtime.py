...
async def request_from_web(request: t.Any, *, max_body_size: int) -> Request:
    """Convert a Web Standards Request-like object into Night Request.

    The adapter expects the small surface exposed by browser/Workers Request:
    ``method``, ``url``, ``headers`` and ``arrayBuffer()``.  Keeping this logic
    duck-typed avoids importing ``js`` or Cloudflare-specific modules in core
    Night code.
    """
    ...
    try:
        ...
        # Pyodide proxies may expose headers as an iterable directly.
    ...
    if len(body) > max_body_size:
        # Request.body() enforces the limit too, but rejecting here avoids
        # retaining an oversized Web Request body in the adapter.
...
def response_to_web(response: Response, *, response_class: t.Any) -> t.Any:
    """Convert a buffered Night Response into a Web Standards Response.

    StreamingResponse needs a separate ReadableStream bridge and is therefore
    deliberately rejected by this first adapter prototype.
    """
    ...
    if raw_headers:
        # Web Headers can represent repeated headers, but a plain dict cannot.
        # Preserve Set-Cookie below where the runtime's Headers implementation
        # supports append().
        try:
...
async def fetch(app: t.Any, request: t.Any, *, response_class: t.Any) -> t.Any:
    """Run a Night app from a Web Standards fetch handler."""
...
class CloudflareWorkerMixin:
    """Small helper for Cloudflare Python Workers.
